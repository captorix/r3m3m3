...
export async function stake(
  ...
  const stakeIx = await program.methods
    ...
    .accounts({
      ...
      smallestStakeEscrow: new PublicKey(
        "Gv4FmrWGp8FwwrNyfCQQjoZSA4Jq1JorViQ3GQ9H5nki"
      ),
...
export async function cancelUnstake(
  ...
  console.log("FWAAFWWAF", remainingAccounts)
  const unstakeIx = await program.methods
    .cancelUnstake()
    .accounts({
...
export async function claimFees(
  ...
  const smallestStakeEscrows: Array<AccountMeta> =
    findReplaceableTopStaker(2, stakeForFee).map((key) => {
      return {
        pubkey: key,
        isWritable: true,
        isSigner: false,
